...
Map::Map()
{
	if (!backgroundTexture.loadFromFile("graphics/background.png")) //chargement du background
	{
	...
	if (!tileSetTexture.loadFromFile("graphics/tileset.png")) //chargement de la tileset
	{
...
void Map::loadMap()
{
	...
	fin.open("map/map.txt"); //on ouvre le fichier map
	...
	while (!fin.eof()) //lecture du fichier jusqu'a la fin
	{
		...
		while (true) //boucle pour lire tout les numero de tiles
		{
			...
			if (!iostr.good()) break; //si c'est finit on quite la boucle
		}
...
void Map::draw(RenderWindow &window)
{
	...
	//on dessine la carte en commancant par startX et startY
	//on commance par y1 jusqu'a y2 (de 0 a 600 pixels)
	//et on se decale de TILE_SIZE (32 pixels) a chaque fois
